...
2026-09-06 · 전체 출시 상태 **PARTIAL**, 실제 Codex 3장 제작 **COMPLETE · 100%**. 새 제품의 G1–G5가 모두 VALID이며 최종 파일 검사와 이미지 검토를 통과했습니다. 브랜드 제외 후 새 ZIP 검증·제품 라이선스 확정·GitHub 발행은 남아 있어 출시 완료를 선언하지 않습니다.
...
후보의 파일 집합과 기술적 공개 범위는 [설치·배포 검증 기록](PACKAGING_CHECKS.md)에 수치와 해시로 기록합니다. 개인 실행 DB·원시 대화·인증 자료·비공개 Git 이력은 제품 ZIP에 포함하지 않습니다. `b3375d79d6d5`는 브랜드 파일 제외 전의 기술 검증 후보이며 최종 공개 배포본이 아닙니다. `244375d`의 NAVER/Jangpm 관련 19개 파일 제외·manifest 경계 회귀는 통과했지만 새 ZIP은 아직 만들지 않았습니다.

현재 최상위 `LICENSE` 파일은 없고 제품의 최상위 라이선스 확정은 진행 중입니다. 출시 담당자가 GitHub API로 새 공개 저장소의 PUBLIC·빈 상태를 확인했으므로 저장소 생성은 **VERIFIED**이며, 코드 push·release는 아직 수행하지 않았습니다. upstream MIT·글꼴 OFL·별도 Python 의존성 조건을 하나의 제품 라이선스로 단정하지 않습니다. PyMuPDF와 브랜드 이미지의 확인 범위는 [제3자 고지](../../THIRD_PARTY_NOTICES.md)를 따릅니다. 이 기록은 법적 적합성 판단이 아닙니다.
