...
3. 저장소 안의 내장 환경을 준비한 뒤 로컬 브라우저 페이지가 열립니다. HTTP 서비스가 시작되면 Codex만 자동으로 확인합니다. 기존 로그인이 있으면 선택 화면 없이 작업실에 연결합니다. 처음이라면 화면의 **Codex 계정으로 로그인**을 눌러 공식 인증 창에서 본인 계정으로 로그인합니다. 인증을 마치면 자동으로 작업실로 이동합니다.
...
현재 MVP는 내장 Codex 전용입니다. 다른 AI를 선택하거나 탐색하지 않으며, 예전에 저장한 선호 도구가 있어도 새 프로젝트는 Codex로 시작합니다. Claude Code 연동은 MVP 이후 추가할 예정입니다. 기존 프로젝트의 실행 기록은 자동 변경하지 않습니다. 세부 범위는 [AI 호환성](AI_COMPATIBILITY.md)을 확인하세요.
...
기본 명령은 `start`, 기본 실행기는 `codex`입니다. 이전 `--provider auto`도 Codex만 확인합니다. `--preflight` 지정 여부와 관계없이 HTTP 시작 시 연결을 확인하며, `--no-runner`는 AI 연결을 시작하지 않습니다. 기본 포트 4317이 사용 중이면 최대 10개 포트 중 사용 가능한 포트를 고릅니다. 같은 프로젝트 작업실이 이미 실행 중이면 확인된 기존 페이지를 엽니다. 실행 터미널의 `Ctrl+C`로 종료합니다.
