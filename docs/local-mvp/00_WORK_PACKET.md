...
출시 상태: **PARTIAL**. 실제 Codex 3장 제작은 **COMPLETE · 100%, G1–G5 VALID**다. 최종 공개 ZIP 검증·제품 라이선스 확정·GitHub 발행은 남아 있다. 아래 완료 기준과 현재 증거 범위를 구분한다.
...
| M7 | **PARTIALLY_VERIFIED** | 이전 ZIP 재현·독립 파일 집합 검사·새 설치 통과, 원본·의존성·폰트 고지 포함. 브랜드 19개 제외·manifest 경계의 최신 21개 회귀 통과. 제외 후 새 ZIP과 제품 라이선스 확정은 남음 |
| M8 | **PARTIALLY_VERIFIED** | 새 공개 저장소 생성은 API의 PUBLIC·빈 상태 확인으로 VERIFIED. 코드 push·release·배포 다운로드는 UNVERIFIED이며 라이선스 확정 대기. 기존 비공개 Git 이력은 발행하지 않음 |
