...
2026-09-06 · **PARTIAL**. 이전 후보 ZIP의 재현성·파일 집합과 새 환경 설치를 확인했습니다. 새 제품의 실제 3장 제작은 G4/G5와 전체 100%까지 통과했습니다. 브랜드 제외 후 새 ZIP·제품 라이선스 확정·GitHub 발행은 남아 있으며 [전체 검증 현황](VERIFICATION.md)과 구분해 관리합니다.
...
최상위 `LICENSE` 파일은 이 후보에 없습니다. 제품 라이선스 확정과 이후 변경을 반영한 최종 배포 검사는 별도 완료 조건입니다.

이후 commit `244375d`에서 NAVER/Jangpm 관련 19개 파일을 공개 배포에서 제외하고 관련 카탈로그·참조와 manifest 경계를 보강했습니다. 최신 21개 회귀는 이 제외 규칙과 변조 거절을 포함합니다. **제외 후 새 ZIP은 아직 생성하지 않았으므로 위 파일 수·SHA·fresh 설치 결과를 새 배포본의 결과로 사용하지 않습니다.** 제외 사유와 조건은 [제3자 고지](../../THIRD_PARTY_NOTICES.md)를 따릅니다.
