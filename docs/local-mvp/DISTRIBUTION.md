...
`.gitattributes`의 `* -text`는 Windows의 `core.autocrlf` 설정과 관계없이 고정된 파일 바이트를 유지합니다. 업스트림 원문 고지의 줄바꿈과 공백도 보존합니다. 설치기는 각 플랫폼의 원본 SHA를 검증한 뒤 저장소 내부 캐시로 해제합니다. 기본 Pretendard 글꼴은 최종 PPTX에 원문 라이선스와 함께 포함하며 OS 글꼴 설치를 요구하지 않습니다.

Windows x64와 Mac ARM64·x64 파일을 모두 포함하므로 배포 ZIP은 약 1.9GB입니다. Git 클론에는 Git 객체 저장 공간도, 첫 실행에는 해당 플랫폼의 압축 해제 공간도 추가로 필요합니다. GitHub Release에는 최종 ZIP과 `SHA256SUMS.txt`를 함께 발행합니다. 검증된 공개 스냅샷을 기본 브랜치에 반영한 뒤 태그를 발행하며, 기존 버전의 태그와 비공개 소스 이력은 변경하지 않습니다.
