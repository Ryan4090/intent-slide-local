...
# Pretendard: Korean + Latin (bundled at assets/fonts/Pretendard/, SIL OFL).
DUAL_SCRIPT_FONTS = {
    'Pretendard', 'Pretendard Thin', 'Pretendard ExtraLight', 'Pretendard Light',
    'Pretendard Medium', 'Pretendard SemiBold', 'Pretendard ExtraBold',
    'Pretendard Black', 'Pretendard JP', 'Pretendard GOV',
}
...
def is_hangul_char(ch: str) -> bool:
    """Check if a character is Korean Hangul (jamo or syllable)."""
    cp = ord(ch)
...
def estimate_text_width(text: str, font_size: float, font_weight: str = '400') -> float:
    ...
    width = 0.0
    for ch in text:
        if is_cjk_char(ch) or is_hangul_char(ch):
            width += font_size
        elif ch == ' ':
            width += font_size * 0.3
        elif ch in 'mMwWOQ%':
            ...
            # Bundled Pretendard digits reach 0.624em regular and 0.682em
            # ExtraBold. 0.65em plus the existing bold factor covers those
            # advances, avoiding unwanted wraps in renderers that ignore
            # wrap="none". Digit widths are not uniformly tabular.
            width += font_size * 0.65
        elif ch.isdigit():
            # isdigit() also includes circled/superscript and other scripts'
            # digits; e.g. Pretendard's circled 3 is about 0.895em bold.
            # They cannot safely inherit the ASCII digit estimate.
            width += font_size
        else:
