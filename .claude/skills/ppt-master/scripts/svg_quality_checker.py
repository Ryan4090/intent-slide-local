...
from gate_receipts import capture_gate, write_gate_pass
...
PPT_SAFE_FONTS = {
    ...
    'impact',
    # Install-local locked family (strategist.md §g install-local font lock):
    # Pretendard is installed user-level on this machine and bundled at
    # assets/fonts/Pretendard/ — treated as PPT-safe for this install.
    'pretendard', 'pretendard light', 'pretendard medium',
    'pretendard semibold', 'pretendard extrabold', 'pretendard black',
    'pretendard thin', 'pretendard extralight',
    # Korean system defaults (Windows/macOS)
    ...
    def _check_pptx_structure_contract(
        ...
        if standard_project and declared_mode != 'structured':
            ...
            self._pptx_structure_issues.append((
                ...
                'template/structured metadata by following .claude/skills/ppt-master/'
        ...
        if check_structure:
            ...
            if native_contract_path.exists() or source_template_path.exists():
                ...
                self._template_issues.append((
                    ...
                    ".claude/skills/ppt-master/workflows/restore-pptx-structure.md",
                ))
            ...
            if legacy_structure_detected:
                self._template_issues.append((
                    ...
                    ".claude/skills/ppt-master/workflows/restore-pptx-structure.md before "
                    "Step 3 consumption",
                ))
...
def main() -> None:
    ...
    if (not template_mode and pages_spec is None and target != '--all'
            and target_path.is_dir() and (target_path / 'svg_output').is_dir()):
        try:
            gate_inputs = capture_gate(
                target_path, 'svg-quality', options={'format': expected_format, 'scope': 'full'}
            )
        except (OSError, ValueError) as exc:
            print(f"[ERROR] Cannot capture gate inputs: {exc}", file=sys.stderr)
            sys.exit(1)
    ...
    # Only complete sweeps produce hash-bound receipts. Partial/template/all
    # checks never authorize verify_deck to skip the full-project gate.
    if gate_inputs is not None and checker.summary['errors'] == 0:
        try:
            write_gate_pass(target_path, 'svg-quality', gate_inputs)
        except ValueError as exc:
            print(f"[ERROR] {exc}", file=sys.stderr)
            sys.exit(1)
        except OSError as exc:
            print(f"[WARN] Cache receipt not written: {exc}", file=sys.stderr)
