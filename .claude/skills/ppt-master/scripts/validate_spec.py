...
from gate_receipts import capture_gate, write_gate_pass  # noqa: E402
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        gate_inputs = capture_gate(project, "spec", options={"strict": args.strict})
    except (OSError, ValueError) as exc:
        print(f"[validate_spec] FAIL: cannot capture gate inputs: {exc}", file=sys.stderr)
        return 1
    ...
    # Hash both inputs and rules; modification times never establish a PASS.
    try:
        write_gate_pass(project, "spec", gate_inputs)
    except ValueError as exc:
        print(f"[validate_spec] FAIL: {exc}", file=sys.stderr)
        return 1
    except OSError as exc:
        print(f"[validate_spec] cache receipt not written: {exc}", file=sys.stderr)
    return 0
