...
**Hard rule — concise spec prose.** `design_spec.md` states decisions in command voice, tables first: no rationale narration beyond one `> Note` line per section, no restating `spec_lock.md` values in prose beyond each section's canonical fields, no pipeline-context essays. §IX per-page entries stay complete (full content / chart / image assignments) — brevity never drops a decision, only the essay around it.

**Hard rule — §II–§VII structure cap.** Each of §II–§VII carries its template tables/fields plus at most one `> Note` line — no added prose paragraphs, no alternative discussions. Omit optional sub-blocks that do not apply (gradient scheme, AI image strategy, §VIII rows on an image-less deck). §VII keeps its audit table with verbatim quotes; runners-up list only genuinely close second-best calls (at least 1 when any viz page exists — no fixed 3-minimum).

**Single-pass confirmation (the default Confirm UI flow; chat mirrors it).** Author every confirmation field in one pass and confirm on one page. The fields still form a dependency order — author them coherently in this sequence:

| Layer | Items | Anchored by |
|---|---|---|
| **direction anchors** | template card *(decks-only; free-design default; leads the page — a deck pick re-defaults the anchors below it)* · template adherence *(only when Step 3 loaded a deck/layout template, or a deck card is selected)* · `a` canvas · `c` key info — audience + `content_divergence` + **delivery purpose** *(PPT only)* (promoted out of `g`) · `d` mode + visual_style | the source + user intent |
| **design system** | `b` page count · `e` color · `f` icon · `g` typography (font + size, formula policy) | the direction anchors |
| **images / execution** | `h` image source + generated-image style · generation mode · refine-spec toggle | direction + design system |

The design-system items are anchored by the direction layer — `visual_style` governs `e` / `f` / `g` (§d Layer 2), and delivery purpose sets the §g body size (one fixed value per purpose), page density, **and** the `b` page-count recommendation. Image strategy follows both layers: h.5 rendering follows the recommended visual direction, while h.5 palette is color behavior only and must follow the §e colors. **Page count is derived, not an anchor** — it follows content volume × delivery purpose. Author the whole bundle against your recommended anchors; when the user confirms different anchors, reconcile per [SKILL.md Step 4](../SKILL.md) (re-derive only the downstream fields the user did not edit — user-set values stay verbatim). The launch / wait mechanics live in [SKILL.md Step 4](../SKILL.md); the item specs below keep their `a`–`h` letters. A staged three-pass confirmation remains available only on explicit user request ([`confirm_ui.md`](../scripts/docs/confirm_ui.md)).
...
> **Default presentation surface — Confirm UI.** Deliver the bundled package through the interactive page: write your recommendations to `<project>/confirm_ui/recommendations.json`, then launch per [SKILL.md Step 4](../SKILL.md). You still author everything — enumerable fields name a recommended `id`; generative fields (color `palette`, CJK + Latin typography, generated-image style) each carry **≥3 distinct candidates**, and the deck's **visual style** (§d Layer 2) carries a **≥3-style personality spectrum** (`visual_style_spectrum`, safe / shifted / bold) — creative recommendations always offer real choice, never a single silent option, same hard rule and thinking as h.5. Honest-shortfall exception (mirrors h.5): if the constraints genuinely cannot yield 3 non-conflicting options, present the smaller set and say why — never pad with duplicates or known-conflicting fillers. A field pinned by a hard lock (the install-local Pretendard typography lock, a template-declared skin) is an honest shortfall by definition: present the single locked plan, not 3 candidates. **Always also print the recommendations + URL in chat** as the always-valid fallback. On confirm, read `<project>/confirm_ui/result.json` (`generation_mode: "split"` / `refine_spec: true` are explicit user choices). Skip the page if the user wants chat-only. Full launch flow, field rules, and JSON schema live in [SKILL.md Step 4](../SKILL.md) + [`scripts/docs/confirm_ui.md`](../scripts/docs/confirm_ui.md) — don't restate them here. The page is a confirmation surface only.

> **Template card recommendation.** The page lists the deck library (synced from `decks_index.json`) plus "free design". Set `recommend.template` to a deck id **only when the deck's summary / use cases genuinely fit the content, audience, and purpose**; otherwise recommend `free`. Free design is the honest default — never over-recommend a deck for merely style-adjacent content (a style wish maps to `visual_style`, not a template). Selecting a deck card re-defaults the direction anchors that deck declares (its `canvas_format` plus the `decks_index.json` `defaults` ids — mode / visual_style / delivery_purpose); switching back to free restores the original recommendations, and every field stays user-editable. A deck confirmed in `result.json` is installed at the confirmation handoff (SKILL.md Step 3 "Deferred install"); its deck-declared skin — including any body-baseline lock or native type ramp — then **MUST** override the color / typography fields the user left at your original recommendation (a color / typography value the user actively edited stays as confirmed); when a template is already installed via an explicit path the card is locked and needs no recommendation.
...
**Delivery purpose** (PPT only) is confirmed here, beside audience, as part of the key information — the deck's consumption mode: `text` (read-close) / `balanced` (business, default) / `presentation`. It is a direction anchor: it sets the §g body size to one fixed value per purpose, plus the type character, page density, and the §b page-count recommendation. Recommend one (`recommend.delivery_purpose`, default `balanced`) and let the user confirm. The fixed body value per purpose lives in §g; the density / treatment side lives in §6.1 — here it is surfaced as a key-information choice, not a separate typography step.
...
- No user description → **present a personality spectrum, not one safe pick** (this is the lever against "every deck looks the same" — the visual style is what most determines a deck's character, so it gets real choice, same hard rule and thinking as h.5). Author **≥3 distinct styles** from the index's auto-selection table spanning *safe* (the industry-norm recommendation) → *shifted* (an alternate one tick more expressive) → *bold* (a characterful style that challenges the default — `brutalist` / `zine` / `memphis` / `ink-wash` / `vintage-poster` etc., whenever the content can carry it). Give each a one-line **temperament tag + real-world analogy** (like h.5's "like an Economist feature"). Write the three to `recommendations.json` `visual_style_spectrum` (each `{id, tag_zh/en/ja/ko, note_zh/en/ja/ko}` — include the `_ja` / `_ko` variants whenever the page `lang` is `ja` / `ko`) **and present the same three in chat** as the always-valid fallback; set `recommend.visual_style` to the *safe* pick as the pre-selected default. The user may pick any of the three, a style outside them, or Custom. Honest-shortfall exception (mirrors h.5): if the content genuinely supports fewer than 3 non-gimmicky directions, present the smaller set and say why — never pad with a style that fights the content.
...
**Template adherence confirmation**: Surface this closed direction-anchor choice only when Step 3 loaded a `kind: deck` or `kind: layout` template. Omit it for free design and brand-only templates.
...
> 4. Search for each concept's filename in the chosen library: `ls .claude/skills/ppt-master/templates/icons/<chosen-library>/ | grep <keyword>`
> 5. Use the verified filename (without `.svg`) as the icon name; always include the library prefix (e.g., `chunk-filled/home`).
> 6. **Copy each chosen icon into the project as you confirm it** — `python3 .claude/skills/ppt-master/scripts/icon_sync.py <project_path> <lib/name> [<lib/name> …]`. This populates `<project>/icons/<lib>/` (the set the Executor embeds from) and, more importantly, **validates existence on the spot**.
...
**Baseline — one fixed value per delivery purpose, not a range.** Delivery purpose is a **direction anchor confirmed in §c key information** (beside audience, not as a separate typography step — see §1 Single-pass confirmation); it is the primary driver because the same canvas reads very differently when read close vs. projected. It is a **deck-wide** axis — beyond the body baseline it also drives page density / count / rhythm; see §6.1 for that side. Here, with the purpose confirmed, it sets the body baseline to a single value:
...
> **Full image system — conditional read.** Illustration roles, the deck motif through-line, sheet→slice planning, the AI path ladder, and **h.5 (rendering × palette lock + three-candidate presentation)** live in [`strategist-images.md`](./strategist-images.md). Read that file before authoring an image recommendation other than `none` / `placeholder`, when supplied assets exist in `images/`, or whenever the confirmed `image_usage` includes `ai` / `web`. A data-only deck recommending `none` skips it entirely.
...
**Delivery purpose drives the whole plan, not just type size.** `result.json delivery_purpose` — `text` (read-close) / `balanced` (business, default) / `presentation`, confirmed as a direction anchor (§1) — is a **deck-wide consumption mode**. It seeds the body baseline (§g) **and** governs how content is distributed:
...
**Default — single-pass authoring for compact sources (may override for bulky / multi-file source sets)**: when the source is one already-in-context file of moderate size (≲ ~300 lines), author the confirmation recommendations, `design_spec.md`, and `spec_lock.md` in one continuous pass — no separate analysis notes and no source re-reads between artifacts.
