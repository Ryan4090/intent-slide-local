...
Codex의 기존 로그인과 설정으로 공통 엔진 v2에 연결하는 한국어 작업실입니다. 현재 MVP는 Codex 전용입니다. 콘솔 자체는 외부 패키지나 CDN에 의존하지 않습니다.

## 의도·조사·디자인 경험

작업실 위쪽의 **제가 이해한 의도**와 **의도를 선명하게 만드는 질문**에서 방향을 먼저 확인합니다. 한 번에 최대 3개 질문에 선택 또는 자유 입력으로 답합니다. 직접 쓴 답변이 선택보다 우선하며, 화면 갱신 중에도 초안을 보존합니다. G1 검토에서 청중의 변화·핵심 메시지·이야기 순서·전달 방식·시각화 원칙과 페이지별 목적을 확인합니다.

조사 중에는 **근거가 모이는 과정**에서 실제 도구의 검색·열람 활동과 현재 유효한 저장 근거를 따로 보여줍니다. 도구 동작 완료는 추출 성공을 뜻하지 않습니다. 사이트·원문·중복 없는 위치 확인 발췌·주장 수는 검증된 조사 결과에서 계산합니다. 원문이 변경되거나 결과가 무효화되면 현재 확보량에서 제외합니다. 상세 활동은 최근 128개 동작을 표시하며 전체 이벤트는 기존 로컬 DB에 보관합니다. **이 기록은 어디에 저장되나요?**에서 실제 DB·원문 폴더·존재하는 조사 결과 묶음의 경로를 확인합니다.

**디자인 10** 탭에서 10개 구성 예시를 보고 표현 방향을 적용할 수 있습니다. 새 프로젝트 기본은 시그널이며 생성창의 표현 방향에서 변경할 수도 있습니다. 진행 중인 작업과 질문을 마친 뒤 선택을 바꿀 수 있고, 제작 단계의 변경은 G3 이후만 다시 검토합니다. 완료한 결과를 바꿀 때는 먼저 디자인 수정 요청을 남깁니다. 의미와 근거는 승인한 의도에서, 시각적 표현은 선택한 디자인에서 가져옵니다. 원본 SVG 제작과 G4/G5 검증 절차는 그대로 유지합니다.
