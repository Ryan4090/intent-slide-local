...
본인의 Codex 또는 Claude Code 정액제 로그인으로 공통 엔진 v2에 연결하는 한국어 작업실입니다. 정적 브리프 생성기인 기존 `web/`과 분리되어 있으며 외부 패키지나 CDN에 의존하지 않습니다.
...
명령은 복제한 Intent-Slide 저장소의 최상위 폴더에서 실행합니다. 첫 지원 OS는 macOS입니다. 설치와 도구 준비는 [제품 시작 안내](../../../README.md)를 따릅니다.
...
1. **AI 연결**에서 Codex 또는 Claude Code를 선택합니다. 설치가 필요한 경우 공식 안내를 열고 터미널에서 본인 계정으로 로그인합니다. 작업실에 비밀번호나 API 키를 붙여넣지 않습니다.
2. **설치·로그인 다시 확인**으로 실제 연결 상태를 확인합니다. API 키 연결이나 미확인 상태를 정액제 준비 완료로 표시하지 않습니다. Claude Code는 **Beta · 실제 슬라이드 제작 검증 대기**로 표시합니다.
3. **새 프로젝트**에서 첫 이야기를 적고 자료 활용 범위와 참고 파일을 선택합니다. 이름은 선택 사항입니다.
4. 생성한 프로젝트에 파일을 모두 첨부한 뒤 첫 의도 작업을 실행합니다. 첨부가 실패하면 실행을 멈추고 이미 만든 프로젝트와 첨부를 보존합니다. 최신 입력을 확인한 뒤 직접 이어서 진행합니다.

프로젝트의 **AI 설정**은 다음 작업부터 적용합니다. 같은 AI를 선택하면 기존 모델·추론 설정을 유지하고, AI를 바꾸면 해당 CLI의 기본 모델을 사용합니다. 작업이 실행 중이거나 답변을 기다리는 동안은 AI를 변경할 수 없습니다. 브라우저는 선택한 AI 이름만 로컬 설정에 저장하고 대화·자격 정보를 저장하지 않습니다.
...
프로젝트 생성의 `execution`은 `{provider, model, effort}`이며 현재 UI는 CLI 기본 모델을 선택합니다. 실행의 `configure_provider` 명령은 같은 형태를 사용합니다. 도구 답변 `provider_answer`는 `request_id`, `response`와 함께 `question_id`, `job_id`, `provider_id`를 모두 전달하여 현재 질문과 실행에 결속합니다.
